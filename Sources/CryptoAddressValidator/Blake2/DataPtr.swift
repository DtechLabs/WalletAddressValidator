...
protocol PtrRepresentable {
    associatedtype Ptr
    func withPtr<R>(cb: (Ptr) throws -> R) rethrows -> R
}

protocol MutPtrRepresentable: PtrRepresentable {
    associatedtype MutPtr
    mutating func withMutPtr<R>(cb: (MutPtr) throws -> R) rethrows -> R
}

protocol DataPtrRepresentable: PtrRepresentable where Ptr == UnsafeRawBufferPointer {}

protocol DataMutPtrRepresentable: MutPtrRepresentable, DataPtrRepresentable
    where MutPtr == UnsafeMutableRawBufferPointer {}

extension Data: DataMutPtrRepresentable {
    typealias Ptr = UnsafeRawBufferPointer
    typealias MutPtr = UnsafeMutableRawBufferPointer
    
    @inlinable func withPtr<R>(cb: (UnsafeRawBufferPointer) throws -> R) rethrows -> R {
        try withUnsafeBytes(cb)
    }
    
    @inlinable mutating func withMutPtr<R>(cb: (UnsafeMutableRawBufferPointer) throws -> R) rethrows -> R {
...
extension Optional: PtrRepresentable where Wrapped: PtrRepresentable {
    typealias Ptr = Optional<Wrapped.Ptr>
    
    func withPtr<R>(cb: (Optional<Wrapped.Ptr>) throws -> R) rethrows -> R {
        switch self {
