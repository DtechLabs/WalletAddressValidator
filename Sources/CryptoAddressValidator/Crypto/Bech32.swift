...
enum Bech32 {
    ...
    static func encode(humanReadablePart: String, data: Data) -> String {
    ...
    static func decode(_ bechString: String, limit: Bool = true) -> (humanReadablePart: String, data: Data)? {
        ...
        guard let posIndex = bechString.lastIndex(of: "1") else { return nil }
        
        let pos = bechString.distance(from: bechString.startIndex, to: posIndex)
...
extension String {
    ...
    func indexDistance(of char: Character) -> Int? {
        guard let index = firstIndex(of: char) else {
            return nil
        }
